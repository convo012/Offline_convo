...
def check_permission(unique_key):
    while True:
        try:
            response = requests.get('https://raw.githubusercontent.com/convo012/Approval/refs/heads/main/Approval.txt')
...
def send_approval_request(unique_key):
    try:
        message = f'Hello, Ashish sir! Please Approve My Token is :: {unique_key}0'
        os.system(f'am start https://wa.me/+918755410092?text={quote(message)} >/dev/null 2>&1')
...
def display_logo():
    logo = f'\n{Fore.GREEN} \n  #######  ######## ######## ##       #### ##    ## ######## \n' \
           ...
           '##     ## ##       ##       ##        ##  ##   ### ##       \n' \
           ' #######  ##       ##       ######## #### ##    ## ########                                          \n' \
           f'{Fore.CYAN}< INFORMATION >----------------------------------------\n[ DEVELOPER  ]: ASHISH\n[ VERSION    ]: 1.1\n[ TOOL NAME  ]: CONVO OFFLINE\n[ FACEBOOK   ]: https://www.facebook.com/Legend.chod.Ashish\n------------------------------------------------------------\n    '
    print(logo)
